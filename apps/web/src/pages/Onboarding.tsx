...
function StepIdentity({ data, onChange }: { data: IdentityData; onChange: (d: IdentityData) => void }) {
  const set = <K extends keyof IdentityData>(k: K, v: IdentityData[K]) => onChange({ ...data, [k]: v });
  return (
    <div className="ob-fields">
      <div className="ob-field">
        <label className="ob-label">你的代号 / 昵称<span className="ob-label-opt">可以填多个</span></label>
        <TagInput value={data.nicknames} onChange={(v) => set("nicknames", v)} placeholder="微信昵称、英文名、花名、备注名…" autoFocus />
      </div>
      <div className="ob-field">
        <label className="ob-label">年龄段 <span className="ob-label-opt">可选</span></label>
        <RadioGroup options={["18-22", "23-27", "28-35", "35+"]} value={data.age_range} onChange={(v) => set("age_range", v)} />
      </div>
      <div className="ob-field">
        <label className="ob-label">职业 / 身份 <span className="ob-label-opt">可选</span></label>
        <input className="ob-input" value={data.occupation} onChange={(e) => set("occupation", e.target.value)} placeholder="产品经理、大学生、设计师…" />
      </div>
      <div className="ob-field">
        <label className="ob-label">MBTI <span className="ob-label-opt">可选</span></label>
        <select className="ob-select" value={data.mbti} onChange={(e) => set("mbti", e.target.value)}>
          <option value="">-- 请选择 --</option>
          {MBTI_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
        </select>
      </div>
      <div className="ob-field">
        <label className="ob-label">星座 <span className="ob-label-opt">可选</span></label>
        <select className="ob-select" value={data.zodiac} onChange={(e) => set("zodiac", e.target.value)}>
          <option value="">-- 请选择 --</option>
...
function StepCommunication({ data, onChange }: { data: CommunicationData; onChange: (d: CommunicationData) => void }) {
  const set = <K extends keyof CommunicationData>(k: K, v: CommunicationData[K]) => onChange({ ...data, [k]: v });
  return (
    <div className="ob-fields">
      <div className="ob-mode-toggle">
        <button className={`ob-mode-btn${data.mode === "simple" ? " active" : ""}`} onClick={() => set("mode", "simple")}>简单填写</button>
        <button className={`ob-mode-btn${data.mode === "complex" ? " active" : ""}`} onClick={() => set("mode", "complex")}>⚡ 提供材料</button>
      </div>
      {data.mode === "complex" ? (
        <div className="ob-field">
          <label className="ob-label">聊天记录 / 日记 / 自述</label>
          <p className="ob-hint">粘贴你和朋友的对话、日记片段或自我描述。系统会自动提取你的说话风格。</p>
          <textarea className="ob-materials-input" rows={10} placeholder="粘贴内容…（建议 200 字以上效果更好）" value={data.materials} onChange={(e) => set("materials", e.target.value)} />
        ...
        <>
          <div className="ob-field">
            ...
            <RadioGroup options={["短句连发", "一整段说完", "视情况而定"]} value={data.message_format} onChange={(v) => set("message_format", v)} />
          </div>
          <div className="ob-field">
            <label className="ob-label">emoji / 表情使用频率</label>
            <RadioGroup options={["经常用", "偶尔用", "基本不用"]} value={data.emoji_usage} onChange={(v) => set("emoji_usage", v)} />
          </div>
          <div className="ob-field">
            <label className="ob-label">标点习惯 <span className="ob-label-opt">可多选</span></label>
            <CheckGroup options={["不用句号", "常用省略号…", "喜欢用～", "全角标点", "无特殊习惯"]} value={data.punctuation_habits} onChange={(v) => set("punctuation_habits", v)} />
          </div>
          <div className="ob-field">
            <label className="ob-label">回复速度倾向</label>
            <RadioGroup options={["秒回", "看心情", "不着急", "已读不回"]} value={data.reply_speed} onChange={(v) => set("reply_speed", v)} />
          </div>
          <div className="ob-field">
            <label className="ob-label">口头禅 / 高频词 <span className="ob-label-opt">可选</span></label>
            <TagInput value={data.catchphrases} onChange={(v) => set("catchphrases", v)} placeholder="哈哈哈、好吧、随便、你认真的吗…" />
          </div>
        </>
      )}
    </div>
...
function StepEmotional({ data, onChange }: { data: EmotionalData; onChange: (d: EmotionalData) => void }) {
  const set = <K extends keyof EmotionalData>(k: K, v: EmotionalData[K]) => onChange({ ...data, [k]: v });
  return (
    <div className="ob-fields">
      <div className="ob-field">
        <label className="ob-label">依恋类型</label>
        <RadioGroup options={["安全型", "焦虑型", "回避型", "不确定"]} value={data.attachment_style} onChange={(v) => set("attachment_style", v)} />
      </div>
      <div className="ob-field">
        <label className="ob-label">爱的语言 <span className="ob-label-opt">最多选 3 项</span></label>
        <CheckGroup options={["肯定的话语", "精心的时刻", "身体接触", "服务行为", "接受礼物"]} value={data.love_languages} onChange={(v) => set("love_languages", v)} max={3} />
      </div>
      <div className="ob-field">
        <label className="ob-label">发生冲突时，你通常</label>
        <RadioGroup options={["冷暴力 / 沉默", "直接说出来", "转移话题", "说反话"]} value={data.conflict_response} onChange={(v) => set("conflict_response", v)} />
      </div>
      <div className="ob-field">
        <label className="ob-label">喜欢一个人时，你会 <span className="ob-label-opt">可选</span></label>
        <input className="ob-input" value={data.when_interested} onChange={(e) => set("when_interested", e.target.value)} placeholder="例如：变得话很多，或者反而更沉默…" />
      </div>
    </div>
  );
}

function StepRelationship({ data, onChange }: { data: RelationshipData; onChange: (d: RelationshipData) => void }) {
  const set = <K extends keyof RelationshipData>(k: K, v: RelationshipData[K]) => onChange({ ...data, [k]: v });
  return (
    <div className="ob-fields">
      <div className="ob-field">
        <label className="ob-label">在关系里，你通常是</label>
        <RadioGroup options={["主动方", "被动方", "看对方", "比较平等"]} value={data.role} onChange={(v) => set("role", v)} />
      </div>
      <div className="ob-field">
        <label className="ob-label">你最看重对方的 <span className="ob-label-opt">最多选 3 项</span></label>
        <CheckGroup options={["真诚", "幽默", "独立", "有边界感", "共同话题", "情绪稳定", "有趣"]} value={data.valued_traits} onChange={(v) => set("valued_traits", v)} max={3} />
      </div>
      <div className="ob-field">
        <label className="ob-label">你的底线 / 不可接受的事 <span className="ob-label-opt">可选</span></label>
        <input className="ob-input" value={data.dealbreakers} onChange={(e) => set("dealbreakers", e.target.value)} placeholder="例如：不能先道歉、不喜欢被催回复…" />
      </div>
    </div>
  );
}
...
function ReviewRow({ label, value }: { label: string; value: string | string[] }) {
  const display = Array.isArray(value) ? (value.length > 0 ? value.join("、") : null) : value || null;
  return (
    <div className="ob-review-row">
      <span className="ob-review-key">{label}</span>
      <span className={`ob-review-val${display ? "" : " empty"}`}>{display ?? "未填写"}</span>
    </div>
  );
}
      ...
      <div className="ob-review-section">
        ...
        {form.communication.mode === "complex" ? (
          <div className="ob-review-row" style={{ flexDirection: "column", alignItems: "flex-start", gap: 4 }}>
            ...
            <span className="ob-review-val" style={{ textAlign: "left", fontSize: 12, whiteSpace: "pre-wrap", maxHeight: 120, overflow: "hidden" }}>
...
function makeEmpty(): FormData {
  return {
    ...
    communication: { mode: "simple", materials: "", message_format: "", emoji_usage: "", punctuation_habits: [], reply_speed: "", catchphrases: [] },
...
export default function Onboarding() {
  ...
  // Pre-populate from saved form.json (edit mode)
  useEffect(() => {
    try {
      const raw = readFile("user/form.json");
      if (raw?.trim()) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const hasMaterials = form.communication.mode === "complex" && form.communication.materials.trim().length > 0;
      ...
      const [personaRes, memoryRes] = await Promise.all([
        chatLearning({ messages: [{ role: "system", content: PERSONA_BUILD_SYSTEM_PROMPT }, { role: "user", content: userMessage }], temperature: 0.4, max_tokens: 1200 }),
        chatLearning({ messages: [{ role: "system", content: USER_MEMORY_BUILD_PROMPT }, { role: "user", content: userMessage }], temperature: 0.3, max_tokens: 800 }),
      ]);

      writeFile("user/persona.md", personaRes.content.trim());
      writeFile("user/memory.md", memoryRes.content.trim());
      writeFile("user/form.json", JSON.stringify(form, null, 2));
  ...
  if (!formReady) {
    return (
      <div className="ob-root">
        <div className="ob-generating"><div className="ob-spinner" /></div>
      </div>
    );
  }
      ...
      <div className="ob-card">
        {!isGenerating && (
          <div className="ob-progress">
            {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
              <>
                <div key={`dot-${i}`} className={`ob-step-dot${i === step ? " active" : i < step || isReview ? " done" : ""}`} />
                {i < TOTAL_STEPS - 1 && <div key={`line-${i}`} className="ob-step-line" />}
              </>
            ))}
          </div>
        )}
        ...
        {isReview && !isGenerating && (
          <div className="ob-header">
            ...
            <p className="ob-subtitle">确认后将生成你的专属 Persona，可随时在 Profile 中更新。</p>
          </div>
        )}
        ...
        {isGenerating ? (
          <div className="ob-generating">
            ...
            <p className="ob-generating-sub">使用 Background Engine 分析你的信息，约需 10-20 秒</p>
          </div>
        ) : isReview ? (
        ...
        {!isGenerating && (
          <div className="ob-nav">
            {step > 0 ? (
              <button className="ob-btn ob-btn-ghost" onClick={handleBack}>返回</button>
            ) : (
              <div className="ob-nav-spacer" />
            )}
            {isReview ? (
              <button className="ob-btn ob-btn-success" onClick={handleGenerate}>生成我的 Persona</button>
            ) : (
              <>
                {step > 0 && step < TOTAL_STEPS && (
                  <button className="ob-btn ob-btn-ghost" onClick={handleNext} style={{ marginLeft: "auto", marginRight: 8 }}>跳过</button>
                )}
                <button className="ob-btn ob-btn-primary" onClick={handleNext} disabled={!canProceed()}>
